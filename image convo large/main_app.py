import wx
import os
import image_utils       # Member 1
import compression_logic # Member 2
import gui_layout        # Member 3
...
class ImageConverterFrame(wx.Frame):
    def __init__(self):
        ...
        # Initialize UI using Member 3's module
        panel = wx.Panel(self)
    ...
    def update_summary(self, original_size_bytes, final_size_bytes):
        if original_size_bytes == 0:
            self.summary_text.SetLabel(gui_layout.initial_summary_text())
            return
    ...
    def on_start_conversion(self, event):hj
    ...
    def process_batch(self, input_paths, output_dir, output_format_pil, quality, to_grayscale, use_target_size, target_size_kb):
        ...
        for idx, input_path in enumerate(input_paths, start=1):
            try:
                ...
                # 2. Grayscale conversion (Image Processing)
                if to_grayscale:
